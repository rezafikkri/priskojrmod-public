...
export default function Nav() {
  ...
  function getActive(pathname, activeClass) {
    if (
      ...
      (currentPathname === '/upgrade-license' || currentPathname === '/download-default-addon')
    ) return activeClass;
        ...
        <ul className="[&>li]:py-2 mb-10 md:mb-0 [&>li]:md:px-4 [&>li]:lg:px-6 md:me-10 lg:me-20 [&>li]:md:inline-block">
          ...
          <li className="relative">
            <button
              ...
              type="button"
              className={getActive('dropdown-menu', 'text-green-500')}
            >
            ...
            {showServices &&
              <ul className="-left-25 md:left-0 -right-3 md:-right-35 lg:-right-30 top-11 md:top-13 absolute bg-black-s [&_a]:block [&_a]:px-5 py-5 space-y-5 rounded-xl shadow-lg">
                <li>
                  <Link
                    className={getActive('/upgrade-license', 'text-green-500')}
                    href="/upgrade-license"
                  >
                    Upgrade License
                  </Link>
                </li>
                <li>
                  <Link
                    href="/download-default-addon"
                    className={getActive('/download-default-addon', 'text-green-500')}
                  >
                    Download Default Addon
                  </Link>
                </li>
              </ul>
            }
